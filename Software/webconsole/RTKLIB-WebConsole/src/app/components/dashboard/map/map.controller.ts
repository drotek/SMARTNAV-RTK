...
export default /*@ngInject*/ function ($scope : angular.IScope, map : IMapService, $rootScope : angular.IRootScopeService, gps : IGpsFactory, $window : angular.IWindowService) {
        ...
        map.getPositions().then((result)=>{
            ...
            if($scope.basePosition.postype === 'llh'){
                ...
                for(var i=0; i<nbPosition; i++){
                    ...
                    var currentLla = gps.eceftolla(parsedPosition);
                    var currentEnu = gps.eceftoenu(baseEcef,parsedPosition,currentLla);
    ...
    /* Loading Process */
    map.getBasePosition().then((position)=>{
